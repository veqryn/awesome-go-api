...
import (
	...
	openapi "github.com/veqryn/awesome-go-api/protobufv3/openapiv3/openapi_generator/go/gen"
)
...
func main() {
	...
	greetingReq := c.DefaultAPI.DefaultGreeting(ctx, "Bob")
	...
	message := "foobar"
	review := openapi.NewAwesomeReviewReq("bob", 4)
	review.Message = &message

	// Create a request, then set the body, before sending
	reviewReq := c.DefaultAPI.DefaultReview(ctx)
	reviewReq = reviewReq.Body(*review) // This returns a copy with the body
	reviewResp, resp, err := reviewReq.Execute()
	if err != nil {
		panic(err)
	}
	...
	errReq := c.DefaultAPI.DefaultError(ctx)
	errorResp, resp, err := errReq.Execute()
	if err != nil {
		spew.Dump(errorResp)
		...
		if resp != nil {
			...
			var errorDetails openapi.RpcStatus
		...
		if errors.As(err, &parsedError) {
			...
			if errorDetails, ok := parsedError.Model().(openapi.RpcStatus); ok {
				spew.Dump(errorDetails)
			}
		} else {
			spew.Dump(resp)
